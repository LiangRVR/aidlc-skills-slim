...
function check(name, status, message) { return { name, status, message }; }
function inspect(root, repaired = []) {
    const checks = [];
    ...
    if (!lockInfo)
        checks.push(check('lock', 'pass', 'No workflow lock present'));
    else if (!lockInfo.lock)
        checks.push(check('lock', 'fail', 'Workflow lock is malformed; doctor --repair can remove it'));
    else if (lockIsProvablyStale(lockInfo.lock))
        checks.push(check('lock', 'warn', `Stale lock from pid ${lockInfo.lock.pid}; doctor --repair can remove it`));
    else
        checks.push(check('lock', 'fail', `Active lock held by pid ${lockInfo.lock.pid} on ${lockInfo.lock.hostname} for ${lockInfo.lock.operation}`));
    if (pendingTransactionExists(root)) {
        try {
            const tx = readPendingTransaction(root);
            checks.push(check('transaction', 'fail', `Pending ${tx.operation} transaction ${tx.id}; doctor --repair can recover it`));
        }
        catch (error) {
            checks.push(check('transaction', 'fail', `Pending transaction is malformed: ${error.message}`));
        }
    }
    else
        checks.push(check('transaction', 'pass', 'No pending transaction'));
    ...
    if (raw === null)
        ...
        try {
            ...
            checks.push(check('state', 'fail', error.message));
        }
    }
    if (state?.active_change) {
        ...
        for (const [name, path] of Object.entries(state.artifacts)) {
            ...
            try {
                ...
                if (name === 'request' && readTextIfExists(root, path) === null)
                    artifactFailures.push(`${name}: missing`);
            }
            catch (error) {
                artifactFailures.push(`${name}: ${error.message}`);
            }
        }
        ...
        try {
            ...
            try {
                ...
                checks.push(check('freshness', 'fail', error.message));
            }
        }
        catch (error) {
            checks.push(check('evidence', 'fail', error.message));
        ...
        try {
            ...
            checks.push(check('check-config', 'fail', error.message));
        }
    }
...
export function doctor(root, repair = false) {
    const repaired = [];
    if (!repair)
    ...
    if (lockInfo) {
        ...
        if (kind !== 'none')
            repaired.push(`removed ${kind} workflow lock`);
    }
    withWorkflowLock(root, 'doctor-repair', () => {
        if (pendingTransactionExists(root)) {
            const operation = recoverPendingTransaction(root);
            if (operation)
                repaired.push(`recovered pending ${operation} transaction`);
        }
        const raw = readStateBytes(root);
        if (raw === null)
            return;
        ...
        if (mutations.length)
            commitTransaction(root, 'doctor-repair', mutations);
    });
    return inspect(root, repaired);
}
