...
function parseArgs(argv) {
    const parsed = { command: argv[0], positional: [], options: new Map() };
...
function optionString(args, name, required = false) {
    const value = args.options.get(name);
    if (typeof value === 'string')
        return value;
    if (required)
        throw new EngineError('USAGE', `Missing --${name}`);
    return undefined;
}
...
function main() {
    ...
    switch (args.command) {
        case 'init': {
            ...
            assertRisk(risk);
            const input = { active_change: optionString(args, 'change', true), risk, risk_rationale: optionString(args, 'risk-rationale', true), workflow: workflowOptions(args), request: optionString(args, 'request', true) };
        ...
        case 'report': {
            ...
            if (!event)
                throw new EngineError('USAGE', 'report requires an event');
            print(engine.report(event));
            break;
        }
        case 'approve':
            print(engine.approve());
            break;
        case 'continue':
            print(engine.continue());
            break;
        case 'request-changes':
            print(engine.requestChanges());
            break;
        case 'block':
            print(engine.block(optionString(args, 'reason', true)));
            break;
        case 'unblock':
            print(engine.unblock(optionString(args, 'reason'), args.options.get('all') === true));
            break;
        case 'cancel':
            print(engine.cancel(optionString(args, 'reason', true)));
            break;
...
try {
    ...
    if (error instanceof EngineError) {
        ...
        process.stderr.write(`${JSON.stringify({ ok: false, error: 'UNEXPECTED', message: error.message }, null, 2)}\n`);
        process.exitCode = 1;
    }
}
