...
export function commitTransaction(root, operation, mutations) {
    ...
    const tx = { transaction_version: 1, engine_version: ENGINE_VERSION, id: randomId(), operation, started_at: new Date().toISOString(), files };
    ...
    try {
        ...
        for (const file of [...applied].reverse()) {
            try { applyContent(root, file.path, file.before); }
            catch { rollbackFailed = true; }
        }
        if (!rollbackFailed) {
            try { deleteRepositoryFile(root, TXN_RELATIVE_PATH); }
            catch { rollbackFailed = true; }
        }
        if (rollbackFailed)
