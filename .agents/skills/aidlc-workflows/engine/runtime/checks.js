...
function normalizeDefinition(name, value) {
    ...
    if (timeout !== undefined && (!Number.isInteger(timeout) || timeout < 1_000 || timeout > MAX_TIMEOUT_MS))
        throw new EngineError('INVALID_CHECK_CONFIG', `Check ${name} timeout_ms must be between 1000 and ${MAX_TIMEOUT_MS}`);
    if (value.cwd !== undefined && (typeof value.cwd !== 'string' || !value.cwd.trim()))
        throw new EngineError('INVALID_CHECK_CONFIG', `Check ${name} cwd must be a non-empty repository-relative path`);
    const allowed = new Set(['command', 'required', 'timeout_ms', 'cwd']);
    const unknown = Object.keys(value).filter((key) => !allowed.has(key));
    if (unknown.length)
        throw new EngineError('INVALID_CHECK_CONFIG', `Check ${name} contains unknown properties`, unknown);
    return { command: [...value.command], required: value.required, ...(timeout !== undefined ? { timeout_ms: timeout } : {}), ...(value.cwd !== undefined ? { cwd: value.cwd.trim() } : {}) };
}
export function readChecksConfig(root) {
    ...
    if (!isObject(parsed) || parsed.schema_version !== 1 || !isObject(parsed.checks))
        throw new EngineError('INVALID_CHECK_CONFIG', `${CHECKS_CONFIG_PATH} must contain schema_version=1 and checks object`);
    const allowed = new Set(['schema_version', 'checks']);
    const unknown = Object.keys(parsed).filter((key) => !allowed.has(key));
    if (unknown.length)
        throw new EngineError('INVALID_CHECK_CONFIG', `${CHECKS_CONFIG_PATH} contains unknown properties`, unknown);
    const checks = {};
    for (const [name, definition] of Object.entries(parsed.checks)) {
        if (!/^[A-Za-z0-9_.-]+$/.test(name))
            throw new EngineError('INVALID_CHECK_CONFIG', `Invalid check name: ${name}`);
        checks[name] = normalizeDefinition(name, definition);
    }
    return { schema_version: 1, checks };
}
export function checksConfigDigest(root) {
    if (!existsSync(assertExistingPathContained(root, CHECKS_CONFIG_PATH)))
        return null;
    return hashArtifact(root, CHECKS_CONFIG_PATH, 'checks config');
}
function checkCwd(root, definition) {
...
export function executeCheck(root, name, config) {
    ...
    if (!definition)
        throw new EngineError('UNKNOWN_CHECK', `Unknown verification check: ${name}`);
    ...
    const checkResult = sourceChanged ? 'STALE' : (!result.error && result.status === 0 ? 'PASS' : 'FAIL');
    return {
...
function sameDefinition(receipt, definition) {
    const cwd = definition.cwd ?? '.';
    return receipt.cwd === cwd && JSON.stringify(receipt.command) === JSON.stringify(definition.command);
}
export function selectVerificationReceipts(evidence, config, sourceDigest) {
    const ids = [];
