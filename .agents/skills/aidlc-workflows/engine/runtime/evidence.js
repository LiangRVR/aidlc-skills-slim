...
export function evidenceRelativePath(change) {
    return `aidlc-docs/changes/${change}/evidence.json`;
}
export function createEvidenceDocument(root, change) {
...
function assertSourceSnapshot(value, label) {
    ...
    for (const record of value.records) {
        ...
        if (record.sha256 !== null)
            assertDigest(record.sha256, `${label}.records.sha256`);
    }
}
function assertManifest(value) {
    if (!Array.isArray(value))
        throw new EngineError('INVALID_EVIDENCE', 'source.manifest must be an array');
    for (const entry of value) {
        ...
        if (entry.sha256 !== null)
            assertDigest(entry.sha256, 'source.manifest.sha256');
    }
}
function assertCheckReceipt(value) {
    ...
    for (const key of ['source_digest_before', 'source_digest_after', 'stdout_sha256', 'stderr_sha256'])
        assertDigest(value[key], `check.${key}`);
}
export function assertValidEvidence(value, expectedChange) {
    ...
    if (expectedChange && value.active_change !== expectedChange)
        throw new EngineError('INVALID_EVIDENCE', `Evidence belongs to ${value.active_change}, expected ${expectedChange}`);
    if (value.source.baseline !== null)
        assertSourceSnapshot(value.source.baseline, 'source.baseline');
    if (value.source.latest !== null)
        assertSourceSnapshot(value.source.latest, 'source.latest');
    assertManifest(value.source.manifest);
    if (!(value.source.manifest_digest === null || typeof value.source.manifest_digest === 'string'))
        throw new EngineError('INVALID_EVIDENCE', 'source.manifest_digest must be digest or null');
    if (value.source.manifest_digest !== null)
        assertDigest(value.source.manifest_digest, 'source.manifest_digest');
    if (!(value.source.unavailable_reason === null || typeof value.source.unavailable_reason === 'string'))
        throw new EngineError('INVALID_EVIDENCE', 'source.unavailable_reason must be string or null');
    if (value.checks.length > MAX_CHECK_RECEIPTS)
        throw new EngineError('EVIDENCE_LIMIT_EXCEEDED', `Evidence contains more than ${MAX_CHECK_RECEIPTS} check receipts`);
    for (const receipt of value.checks)
        assertCheckReceipt(receipt);
}
export function serializeEvidence(evidence) {
    ...
    if (Buffer.byteLength(serialized, 'utf8') > MAX_EVIDENCE_BYTES)
        throw new EngineError('EVIDENCE_LIMIT_EXCEEDED', `evidence.json exceeds ${MAX_EVIDENCE_BYTES} bytes`);
    return serialized;
}
export function readEvidence(root, state) {
    if (!state.active_change || !state.evidence_path)
        throw new EngineError('INVALID_STATE', 'Active workflow requires evidence_path');
    const raw = readTextIfExists(root, state.evidence_path, MAX_EVIDENCE_BYTES);
    if (raw === null)
        throw new EngineError('EVIDENCE_MISSING', `Evidence file is missing: ${state.evidence_path}`);
    const parsed = parseJson(raw, state.evidence_path);
    assertValidEvidence(parsed, state.active_change);
    return parsed;
}
export function updateSourceEvidence(root, evidence, current) {
...
export function appendCheckReceipt(evidence, receipt) {
    if (evidence.checks.some((item) => item.receipt_id === receipt.receipt_id))
        throw new EngineError('INVALID_EVIDENCE', `Duplicate check receipt id: ${receipt.receipt_id}`);
    if (evidence.checks.length >= MAX_CHECK_RECEIPTS)
        throw new EngineError('EVIDENCE_LIMIT_EXCEEDED', `Cannot append more than ${MAX_CHECK_RECEIPTS} check receipts`);
    return { ...evidence, checks: [...evidence.checks, receipt] };
}
export function selectedEvidenceDigest(evidence, receiptIds) {
    ...
    const selected = [...receiptIds].sort().map((id) => {
        ...
        if (!receipt)
            throw new EngineError('STALE_EVIDENCE', `Verification references missing check receipt ${id}`);
        return receipt;
    });
