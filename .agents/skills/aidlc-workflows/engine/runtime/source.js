...
function testOnlyEmptySnapshot() {
    const records = [];
    return {
        snapshot_version: 1,
        mode: 'git',
...
function ensureGitRepository(root) {
    if (isGitRepository(root))
        return;
    if (process.env.AIDLC_TEST_ALLOW_NON_GIT === '1')
        return;
...
function baseTree(root) {
    ...
    if (!result.error && result.status === 0)
        return { tree: (result.stdout ?? '').trim(), hasHead: true };
    return { tree: EMPTY_TREE, hasHead: false };
}
function hashSourceFile(root, path) {
    const lexical = assertExistingPathContained(root, path);
    if (!existsSync(lexical))
        return null;
    const stats = statSync(lexical);
    if (!stats.isFile())
        throw new EngineError('SOURCE_SNAPSHOT_UNAVAILABLE', `Source path is not a regular file: ${path}`);
    if (stats.size > MAX_SOURCE_FILE_BYTES)
        throw new EngineError('SOURCE_FILE_TOO_LARGE', `${path} exceeds ${MAX_SOURCE_FILE_BYTES} bytes`);
...
function parseNameStatus(raw) {
    ...
    for (let i = 0; i < tokens.length;) {
        ...
        if (status.includes('\t')) {
            ...
            path = tokens[i++];
        if (!path)
            throw new EngineError('SOURCE_SNAPSHOT_UNAVAILABLE', 'Unable to parse git name-status output');
        if (status.startsWith('R') || status.startsWith('C')) {
            const newPath = tokens[i++];
            if (!newPath)
                throw new EngineError('SOURCE_SNAPSHOT_UNAVAILABLE', 'Unable to parse git rename/copy output');
...
function collectRecords(root, hasHead) {
    const records = [];
    if (hasHead) {
        const raw = runGit(root, ['diff', '--name-status', '-z', '-M', 'HEAD', '--']);
        for (const item of parseNameStatus(raw)) {
            if (isWorkflowPath(item.path) && (!item.previous_path || isWorkflowPath(item.previous_path)))
                continue;
    ...
    for (const path of untrackedRaw.split('\0').filter(Boolean)) {
        if (isWorkflowPath(path) || known.has(path))
            continue;
        records.push({ status: '??', path, previous_path: null, sha256: hashSourceFile(root, path) });
    }
    if (records.length > MAX_SOURCE_RECORDS)
        throw new EngineError('SOURCE_MANIFEST_TOO_LARGE', `Source snapshot has more than ${MAX_SOURCE_RECORDS} changed/untracked records`);
...
export function tryCaptureSourceSnapshot(root) {
    try {
        return { snapshot: captureSourceSnapshot(root), reason: null };
    }
    catch (error) {
        return { snapshot: null, reason: error instanceof Error ? error.message : String(error) };
    }
}
...
export function buildSourceManifest(root, baseline, current) {
    if (!baseline) {
    ...
    for (const [path, record] of currentRecords) {
        ...
        if (!old || old.status !== record.status || old.previous_path !== record.previous_path || old.sha256 !== record.sha256)
            candidates.set(path, record);
    }
    for (const [path, old] of baselineRecords) {
        if (!currentRecords.has(path))
            candidates.set(path, { status: 'X', path, previous_path: old.previous_path });
    }
    ...
    for (const item of [...candidates.values()].sort((a, b) => a.path.localeCompare(b.path))) {
        ...
        if (baselineDirty && baselineDirty.sha256 === currentHash && baselineDirty.previous_path === item.previous_path)
            continue;
        manifest.push({ status: statusName(item.status), path: item.path, previous_path: item.previous_path, sha256: currentHash });
    }
    if (manifest.length > MAX_SOURCE_RECORDS)
        throw new EngineError('SOURCE_MANIFEST_TOO_LARGE', `Source manifest has more than ${MAX_SOURCE_RECORDS} entries`);
