...
export function acquireWorkflowLock(root, operation) {
    ...
    try {
        ...
        if (error?.code !== 'EEXIST')
            throw new EngineError('LOCK_FAILED', `Unable to acquire workflow lock: ${error?.message ?? String(error)}`);
        const existing = readWorkflowLock(root);
        if (existing?.lock)
            throw new EngineError('WORKFLOW_LOCKED', `Workflow is locked by pid ${existing.lock.pid} on ${existing.lock.hostname} for ${existing.lock.operation}`);
