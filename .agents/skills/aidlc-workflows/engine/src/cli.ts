...
function main(): void {
  ...
  switch (args.command) {
    ...
    case 'freshness': print(engine.freshness()); break;
    case 'refresh': print(engine.refresh()); break;
    ...
    case 'check': {
      const name = args.positional[0]; if (!name) throw new EngineError('USAGE', 'check requires a configured check name');
      print(engine.check(name)); break;
    }
    case 'report': {
