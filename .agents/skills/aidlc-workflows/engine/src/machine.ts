...
export function createInitialState(input: InitInput, revision = 1): AidlcState {
  const change = input.active_change.trim();
  if (!change || change.includes('/') || change.includes('\\') || change === '.' || change === '..') throw new EngineError('INVALID_ARGUMENT', 'active_change must be a non-empty single path segment');
...
export function applyEvent(root: string, current: AidlcState, event: LifecycleEvent): AidlcState {
  ...
  switch (event) {
    case 'requirements_complete':
      ...
      if (next.workflow.design_required) { next.stage = 'design'; activeProgress(next, 'design'); } else { next.stage = 'plan'; activeProgress(next, 'plan'); }
      break;
    case 'design_complete':
      requireState(current, 'design', ['active'], event); checkDesign(root, current); next.progress.design = 'complete'; next.stage = 'plan'; activeProgress(next, 'plan'); break;
    case 'plan_complete':
      ...
      else { next.stage = 'implementation'; next.status = 'active'; next.gate = null; activeProgress(next, 'implementation'); }
      break;
    case 'approve':
    ...
    case 'request_changes':
      if (current.stage === 'plan' && ['awaiting_approval', 'approved_hold'].includes(current.status)) {
        next.stage = 'plan'; next.status = 'active'; next.gate = null; next.progress.plan = 'active';
      } else if (current.stage === 'final_acceptance' && current.status === 'awaiting_acceptance') {
        next.stage = 'implementation'; next.status = 'active'; resetDownstreamAfterImplementationReopen(next);
      } else throw new EngineError('ILLEGAL_TRANSITION', `request_changes is illegal from ${current.stage}/${current.status}`);
      break;
    case 'implementation_complete':
      ...
      if (next.workflow.review_required) { next.stage = 'review'; activeProgress(next, 'review'); } else { next.stage = 'verification'; activeProgress(next, 'verification'); }
      break;
    case 'review_pass':
      requireState(current, 'review', ['active'], event); checkReviewPass(root, current); next.progress.review = 'complete'; next.stage = 'verification'; activeProgress(next, 'verification'); break;
    case 'review_fail':
      requireState(current, 'review', ['active'], event); checkReviewFail(root, current); next.stage = 'implementation'; next.status = 'active'; resetDownstreamAfterImplementationReopen(next); break;
    case 'verification_complete':
      ...
      else { next.stage = 'complete'; next.status = 'complete'; next.gate = null; }
      break;
    case 'accept':
...
export function cancelState(current: AidlcState, reason: string): AidlcState {
  assertValidState(current);
