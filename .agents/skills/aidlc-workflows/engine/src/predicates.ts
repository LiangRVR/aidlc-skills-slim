...
function artifactPath(root: string, path: string | null, label: string): string {
  if (!path) throw new EngineError('PREDICATE_FAILED', `${label} artifact path is missing`);
  if (isAbsolute(path)) throw new EngineError('PREDICATE_FAILED', `${label} artifact path must be repository-relative`);
  const rootAbs = resolve(root);
  const fileAbs = resolve(rootAbs, path);
  const rel = relative(rootAbs, fileAbs);
  if (rel.startsWith('..' + sep) || rel === '..' || isAbsolute(rel)) {
    throw new EngineError('PREDICATE_FAILED', `${label} artifact path escapes repository root`);
  }
  return fileAbs;
}

function readArtifact(root: string, path: string | null, label: string): string {
  const file = artifactPath(root, path, label);
  if (!existsSync(file)) throw new EngineError('PREDICATE_FAILED', `${label} artifact does not exist: ${path}`);
...
export function checkRequirements(root: string, state: AidlcState): void {
  ...
  for (const heading of ['Intent', 'Acceptance Criteria', 'Must Preserve', 'Constraints', 'Out of Scope']) {
    if (!hasHeading(text, heading)) throw new EngineError('PREDICATE_FAILED', `requirements.md is missing required section: ${heading}`);
  }
...
export function checkDesign(root: string, state: AidlcState): void {
  ...
  for (const heading of ['Context', 'Proposed Design', 'Project Baseline Impact']) {
    if (!hasHeading(text, heading)) throw new EngineError('PREDICATE_FAILED', `design.md is missing required section: ${heading}`);
  }
}
...
export function checkPlan(root: string, state: AidlcState): void {
  ...
  for (const heading of ['Scope', 'Work', 'Verification Strategy']) {
    if (!hasHeading(text, heading)) throw new EngineError('PREDICATE_FAILED', `plan.md is missing required section: ${heading}`);
  }
...
export function checkVerification(root: string, state: AidlcState): void {
  ...
  for (const heading of ['Requirement Coverage', 'Automated Checks', 'Runtime / User-Surface Checks', 'Baseline Consistency', 'Limitations / Residual Risk']) {
    if (!hasHeading(text, heading)) throw new EngineError('PREDICATE_FAILED', `verification.md is missing required section: ${heading}`);
  }
  ...
  for (const id of ids) {
    ...
    if (/\bNOT\s+VERIFIED\b/i.test(row)) hasNotVerified = true;
    else if (!/\bPASS\b/i.test(row)) missing.push(id);
  }
