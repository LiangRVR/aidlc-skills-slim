...
import type { AidlcState, DoctorReport, InitInput, LifecycleEvent, NextDirective } from './types.js';
...
export class AidlcEngine {
  ...
  init(input: InitInput): AidlcState {
    return withWorkflowLock(this.root, 'init', () => {
      ...
      if (raw !== null) {
        ...
        if (schemaVersionOf(parsed) === 1) throw new EngineError('STATE_MIGRATION_REQUIRED', 'Existing schema v1 state must be migrated before initialization');
  ...
  next(): NextDirective { return nextDirective(this.status()); }
  ...
  migrate(): AidlcState {
    return withWorkflowLock(this.root, 'migrate', () => {
      ...
      const parsed = parseJson(raw, STATE_RELATIVE_PATH);
      if (schemaVersionOf(parsed) === 2) { assertValidState(parsed); return parsed; }
