...
import type { AidlcState, CheckReceipt, DoctorReport, EvidenceDocument, FreshnessIssue, InitInput, LifecycleEvent, NextDirective } from './types.js';
...
export class AidlcEngine {
  ...
  init(input: InitInput): AidlcState {
    return withWorkflowLock(this.root, 'init', () => {
      ...
      let existing: AidlcState | null = null;
      if (raw !== null) {
        const parsed = parseJson(raw, STATE_RELATIVE_PATH);
        if (schemaVersionOf(parsed) !== 3) throw new EngineError('STATE_MIGRATION_REQUIRED', 'Existing state must be migrated to schema v3 before initialization');
        assertValidState(parsed); existing = parsed;
      }
      if (existing && !['complete', 'cancelled'].includes(existing.stage)) throw new EngineError('ACTIVE_CHANGE_EXISTS', `Active change already exists: ${existing.active_change}`);
      if (changeDirectoryExists(this.root, input.active_change)) throw new EngineError('CHANGE_EXISTS', `Change directory already exists: ${input.active_change}`);
  ...
  status(): AidlcState {
    if (pendingTransactionExists(this.root)) throw new EngineError('RECOVERY_REQUIRED', 'Pending workflow transaction exists; run doctor --repair');
  ...
  next(): NextDirective {
    ...
    if (state.status === 'blocked' || state.stage === 'cancelled') return base;
  ...
  doctor(repair = false): DoctorReport { return runDoctor(this.root, repair); }

  migrate(): AidlcState {
    return withWorkflowLock(this.root, 'migrate', () => {
      ensureNoPendingTransaction(this.root);
      const raw = readStateBytes(this.root);
      if (raw === null) throw new EngineError('NO_STATE', 'No AI-DLC state exists in this project');
      const parsed = parseJson(raw, STATE_RELATIVE_PATH);
      if (schemaVersionOf(parsed) === 3) { assertValidState(parsed); return parsed; }
      ...
      if (evidence && next.evidence_path && readTextIfExists(this.root, next.evidence_path) === null) mutations.push({ path: next.evidence_path, after: serializeEvidence(evidence) });
  ...
  report(event: LifecycleEvent): AidlcState {
    if (['approve', 'continue', 'request_changes'].includes(event)) throw new EngineError('INVALID_ARGUMENT', `Use the dedicated ${event.replace('_', '-')} command for ${event}`);
  ...
  check(name: string): CheckReceipt {
    return withWorkflowLock(this.root, `check:${name}`, () => {
      ensureNoPendingTransaction(this.root);
      const current = this.readCurrentUnlocked();
      if (current.stage !== 'verification' || current.status !== 'active') throw new EngineError('ILLEGAL_TRANSITION', 'Executable checks run only during active Verification');
      const evidence = readEvidence(this.root, current);
      ...
      const next = this.bump(JSON.parse(JSON.stringify(current)) as AidlcState, current);
  ...
  reclassify(input: ReclassifyInput): AidlcState {
    return withWorkflowLock(this.root, 'reclassify', () => {
      ...
      if (changed.progress.design !== 'complete') changed.freshness.design = null;
  ...
  cancel(reason: string): AidlcState {
    ...
    return withWorkflowLock(this.root, event, () => {
      ...
      switch (event) {
        case 'requirements_complete':
          ...
          next.freshness.design = null; next.freshness.plan = null; next.freshness.planning_approval = null; next.freshness.implementation = null; next.freshness.review = null; next.freshness.verification = null; next.freshness.final_acceptance = null;
          break;
        case 'design_complete':
          next.freshness.design = buildDesignReceipt(this.root, current, at);
          next.freshness.plan = null; next.freshness.planning_approval = null; next.freshness.implementation = null; next.freshness.review = null; next.freshness.verification = null; next.freshness.final_acceptance = null;
          break;
        case 'plan_complete':
          next.freshness.plan = buildPlanReceipt(this.root, current, at);
          next.freshness.planning_approval = null; next.freshness.implementation = null; next.freshness.review = null; next.freshness.verification = null; next.freshness.final_acceptance = null;
          break;
        case 'approve': {
        ...
        case 'continue': {
          ...
          if (approval && approval.plan_bundle_sha256 !== plan.bundle_sha256) throw new EngineError('STALE_APPROVAL', 'Planning approval does not match the current Plan fingerprint');
        ...
        case 'request_changes':
          if (current.stage === 'plan') {
            ...
            next.freshness.implementation = null; next.freshness.review = null; next.freshness.verification = null; next.freshness.final_acceptance = null;
          }
          break;
        case 'implementation_complete': {
          ...
          if (current.workflow.planning_gate_required) {
            const approval = current.freshness.planning_approval;
            if (!approval || approval.plan_bundle_sha256 !== plan.bundle_sha256 || !approval.continued_at) throw new EngineError('STALE_APPROVAL', 'Implementation requires a current, continued planning approval receipt');
          }
          ...
          if (!manifestDigest) throw new EngineError('SOURCE_SNAPSHOT_UNAVAILABLE', 'Source manifest digest was not produced');
          next.freshness.implementation = buildImplementationReceipt(plan, source.digest, manifestDigest, at);
          next.freshness.review = null; next.freshness.verification = null; next.freshness.final_acceptance = null;
          break;
        }
        case 'review_pass': {
          const implementation = current.freshness.implementation;
          if (!implementation) throw new EngineError('STALE_EVIDENCE', 'Review requires an Implementation source receipt');
          next.freshness.review = buildReviewReceipt(this.root, current, implementation.source_digest, at);
          next.freshness.verification = null; next.freshness.final_acceptance = null;
          break;
        }
        case 'review_fail':
          next.freshness.implementation = null; next.freshness.review = null; next.freshness.verification = null; next.freshness.final_acceptance = null;
        ...
        case 'verification_complete': {
          const implementation = current.freshness.implementation;
          if (!implementation) throw new EngineError('STALE_EVIDENCE', 'Verification requires an Implementation source receipt');
          ...
          if (selected.missingRequired.length) throw new EngineError('VERIFICATION_EVIDENCE_MISSING', 'Required executable checks do not have current passing receipts', selected.missingRequired);
        ...
        case 'accept': {
          ...
          if (!verification) throw new EngineError('STALE_EVIDENCE', 'Final Acceptance requires a current Verification receipt');
      ...
      if (bumped.evidence_path) mutations.push({ path: bumped.evidence_path, after: serializeEvidence(evidence) });
  ...
  private assertFresh(state: AidlcState, evidence: EvidenceDocument): void {
    const issues = inspectFreshness(this.root, state, evidence);
    if (issues.length) throw new EngineError('FRESHNESS_STALE', 'Workflow has stale dependencies; run aidlc-engine refresh before continuing', issues.map((issue) => `${issue.dependency}: ${issue.message}`));
  }

  private requirePlanReceipt(state: AidlcState) {
    if (!state.freshness.plan) throw new EngineError('STALE_EVIDENCE', 'Current Plan has no completion fingerprint; run refresh and complete Plan again');
    return state.freshness.plan;
  }

  private readCurrentUnlocked(): AidlcState {
    const state = readState(this.root);
    if (!state) throw new EngineError('NO_STATE', 'No AI-DLC state exists in this project');
    return state;
  }

  private bump(next: AidlcState, current: AidlcState): AidlcState {
