...
3. **Offer adjustment menu**: Call the `question` tool with:
   ...
   - **Option B**: 设置需要人工审核的阶段 / Set review stages — present a multi-select (`multiple: true`) question listing ALL stage names (Workspace Detection, Reverse Engineering, Requirements Analysis, User Stories, Workflow Planning, Application Design, Units Generation, Functional Design, NFR Requirements, NFR Design, Infrastructure Design, Code Generation, Build and Test); selected stages go into the Review Stages list (replacing the previous list); the user may also name stages freely via custom input
