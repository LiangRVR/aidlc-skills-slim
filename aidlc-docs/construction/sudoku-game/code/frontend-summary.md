...
- `src/scenes/menu-scene.ts` — 主菜单：标题、"继续上次游戏"（hasSave 时，load 失败降级回菜单）、"新游戏" → 四档难度按钮（简单/中等/困难/专家）；按钮组按总数垂直居中，起点钳制在标题下方（第四轮起改为双页滑动导航：第一屏模式选择 → 滑至第二屏难度选择 + 返回，见 sudoku-online-client/code/client-summary.md）
- `index.html` — 游戏容器 `data-testid="game-container"`；body flex 使画布水平居中，页面背景与画布统一为 `#fafafa`
...
- `src/ui/board-view.ts` — 468px 棋盘（52px 格）：宫线粗/细线区分；高亮优先级 冲突 > 选中 > 同数字 > 关联行列宫；预填格黑色粗体、用户填写蓝色、错误红色（BR-06）；笔记 3x3 小字渲染，选中格含数字 N 时全盘笔记中的 N 加粗（实施期变更）
...
- `src/ui/vfx-manager.ts` — 连击特效（第二轮 FR-13 / BR-26~BR-30）：streak 计数与 4 档分层（1-2/3-5/6-8/9+）；填对时格子粒子爆发 + 光环扩散 + 闪格，完成行/列/宫时区域扫光；连击 ≥3 棋盘边框脉冲、≥6 叠加上升粒子、≥9 叠加沿屏幕边缘漂浮的粒子框（EdgeZone 发射器，替代原 vignette 色条）与彩纸；填错时破碎粒子 + 震屏，连击清零且全部氛围特效消失；reset/destroy 供重开与场景销毁调用
...
- 第三轮（FR-14 专家难度）后复验：`npx tsc --noEmit` 0 错误、`npx vitest run` 60/60 通过、`npm run build` 成功
